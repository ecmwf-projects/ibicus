...
class ISIMIP(Debiaser):
    """
    Implements the ISIMIP3b and ISIMIP3BASD bias adjustment methodology following Lange 2019 and Lange 2021.

    ISIMIP is a debiasing methodology covering a variety of variables: ["hurs", "pr", "prsnratio", "psl", "rsds", "rlds", "sfcwind", "tas", "tasrange", "taskew"]. From those also tasmin, tasmax and prsn can be derived as: tasrange = tasmin - tasmax, tasskew = (tas-tasmin)/(tasmax-tasmin) and prsnratio = prsn/pr.
    The ISIMIP methodology consists of a variety of 8 steps with particularities for several variables. We refer to Lange 2019 for a more complete description. In the following the steps shall only be sketched.
    Let obs refer to the observation dataset and cm_hist and cm_fut to historical and future climate model values.

    Steps:

    1. If scale_by_annual_cycle_of_upper_bounds = True (for rsds only) then an annual cycle of upper bounds is calculated to scale the values in obs, cm_hist and cm_fut onto the interval [0,1]. This is to account physical upper bounds that vary over the year, given that other variables have static upper and lower bounds.

    2. If impute_missing_values = True (for prsnration only) missing values (passed in either as masked array or array with np.nan values) in obs, cm_hist and cm_fut are imputed via somewhat trend preserving sampling from all other non-missing values within a given time window.

    3. If detrending = True (for tas, psl and rlds) the timeseries are detrended to account for trends within historical and future period (not for trends between historical and future one as this is done in step 5). Trends are estimated via linear regression on the annual means and then removed from the daily observations. If detrending_with_significance_test = True then a significance test is first used to determine whether the trend is to be removed.

    4. If the variable has either a lower or upper bound and threshold values beyond the threshold are randomized within between the respective bound and threshold. This allows to quantile map these values in step 6. Randomization is done uniformly (different than from Lange 2019 to reflect a change in ISIMIP3b v2.2), however ranks are preserved.

    5. Here pseudo future observations are generated by transferring a climate change signal onto the observations. This is done differently depending on the trend_preservation_method: one of ["additive", "multiplicative", "mixed", "bounded"] to reflect differences in the trends of different variables. For bounded variables if trend_transfer_only_for_values_within_threshold = True then trend transfer only happens for the values within thresholds, else for all values.

    6. Given the pseudo future observations from step 5 a quantile mapping is used mapping the values of cm_fut to the pseudo future observations to generate debiased future climate model values. If event_likelihood_adjustment = True also an event likelihood adjustment is done, following Switanek et al. 2017 and as originally described in Lange 2019, but removed in the ISIMIP3b run. By default the quantile mapping is done parametically using the distribution in self.distribution, however if nonparametric_qm = True or ks_test_for_goodness_of_cdf_fit = True and a Kolmogorov Smirnov statistic indicats a bigger misfit a nonparametric quantile mapping is used instead (default for hurs, psrnratio, rsds, tasskew). If bias_correct_frequencies_of_values_beyond_thresholds = True (default) also the frequencies of values beyond thresholds are adjusted to. This adjusts eg. the number of precipitation dry days.

    7. If detrending = True then the trend removed in step 3 is restored onto the debiased cm_fut.

    8. If scale_by_annual_cycle_of_upper_bounds = True (for rsds only) then a debiased annual cycle of upper bounds is calculated to rescale the debiased climate model values from [0,1] back to the original scale.
    ...
    All variables have different default settings and using the ISIMIP.from_variable method it is possible to initialize a debiaser with these settings eg debiaser = ISIMIP.from_variable("tas"). However depending on the debiasing region and usecase at hand it might be important to modify some of the default settings. This can be done using something like
    ...
    If run with the default options results are exactly identical to v3.0 (2022-04-28) and higher of the [ISIMIP3BASD reference implementation](https://zenodo.org/record/6758997/files/isimip3basd-master.tar.gz). However we include further options for customization of the ISIMIP behavior, allow more flexible usage of the code given that it operates on a numerical basis and handle a variety of cases that might lead to bugs in the reference implementation (TODO).
    ...
    **References**:
    ...
    Required and most important attributes (without defaults):
    ----------------------------------------------------------
    ...
    distribution: Union[scipy.stats.rv_continuous, scipy.stats.rv_discrete, scipy.stats.rv_histogram, StatisticalModel]
        Distribution or statistical model used for the quantile mapping in step 6 if nonparametric_qm = False (default)
    ...
    nonparametric_qm: bool
        Whether nonparametric quantile mapping is used for debiasing in step 6. If nonparametric_qm = False then quantile mapping is done using the distribution in self.distribution. Default: false
    detrending: bool
        Whether detrending is applied in step4 to account for within obs, cm_hist and cm_fut trends. Default: False (TODO).
    ...
    lower_bound: float
        Lower bound of a variable. Default: -np.inf.
    lower_threshold: float
        Lower threshold of a variable. Default: -np.inf.
    upper_bound: float
        Upper bound of a variable. Default: np.inf.
    upper_threshold: float
        Upper threshold of a variable. Default: np.inf.

    Step 1:
    scale_by_annual_cycle_of_upper_bounds: bool
        Whether in step 1 a scaling by an estimated annual cycle of upper bounds is done to scale obs, cm_hist and cm_fut values onto [0,1]. Default: False.
    window_length_annual_cycle_of_upper_bounds: int
        Window length to estimate an annual cycle of upper bounds to rescale. Only relevant if scale_by_annual_cycle_of_upper_bounds = True. Default: 31.

    Step 2:
    impute_missing_values: bool
        Whether in step 2 missing values (indicated either by a mask of np.nan values) are imputed using the other values within the window.

    Step 3:
    detrending: bool
        Whether detrending is applied in step4 to account for within obs, cm_hist and cm_fut trends. Default: False (TODO).
    detrending_with_significance_test: bool
        Whether detrending is applied conditional on a significance test for the trend. Only relevant if detrending = True.

    Step 5:
    trend_preservation_method: str
        One of ["additive", "multiplicative", "mixed", "bounded"]. Method of how the climate change trend is transferred to observations to generate the pseudo future observations.
    trend_transfer_only_for_values_within_threshold: bool
        Whether the trend transfer is done only for the values within thresholds. Default: True.
    ...
    bias_correct_frequencies_of_values_beyond_thresholds: bool
        Whether frequencies of values beyond thresholds are adjusted/bias corrected. Default: True.
    event_likelihood_adjustment: bool
        Whether in conjuction with quantile mapping an event likelihood adjustment is used inspired by Switanek et al. 2017. Default: False.
    ks_test_for_goodness_of_cdf_fit: bool
        Whether a Kolmogorov Smirnov statistic is used to diagnose misfit of the parametric CDF prior to quantile mapping. If such misfit is diagnosed nonparametric quantile mapping is used. Only relevant if nonparametric_qm = False. Default: True.
    ...
    mode_non_parametric_qm: str
        One of ["normal", "isimipv3.0"]. Computation method used for nonparametric quantile mapping. If "normal" this is based on estimated empirical CDFs and inverse empirical CDFs. If "isimipv3.0" this is based on estimated ranks using scipy.stats.rankdata which can lead to inconsistencies. Default: "normal"

    Iteration:
    running_window_mode: bool
        Whether ISIMIP is used in running window mode to account for seasonalities. If running_window_mode = False then ISIMIP is applied by months. Default: True.
    running_window_length: int
        Length of the running window in days: how many values are used to the debiased climate model values. Only relevant if running_window_mode = True. Default: 31.
    running_window_step_length: int
        Step length of the running window in days: how many values are debiased inside the running window. Only relevant if running_window_mode = True. Default: 1
    """
