...
class ISIMIP(Debiaser):
    ...
    ISIMIP is a debiasing methodology covering a variety of variables::

        ["hurs", "pr", "prsnratio", "psl", "rsds", "rlds", "sfcwind", "tas", "tasrange", "taskew"]

    From those also tasmin, tasmax and prsn can be derived as: ``tasrange = tasmin - tasmax, tasskew = (tas-tasmin)/(tasmax-tasmin)`` and ``prsnratio = prsn/pr``.
    ...
    **Steps:**
    ...
    1. :py:func:`step1`: If ``scale_by_annual_cycle_of_upper_bounds = True`` (for rsds only) then an annual cycle of upper bounds is calculated to scale the values in obs, cm_hist and cm_fut onto the interval [0,1]. This is to account physical upper bounds that vary over the year, given that other variables have static upper and lower bounds. |brr|

    2. :py:func:`step2`: If ``impute_missing_values = True`` (for prsnration only) missing values (passed in either as masked array or array with np.nan values) in obs, cm_hist and cm_fut are imputed via somewhat trend preserving sampling from all other non-missing values within a given time window. |brr|

    3. :py:func:`step3`: If ``detrending = True`` (for tas, psl and rlds) the timeseries are detrended to account for trends within historical and future period (not for trends between historical and future one as this is done in step 5). Trends are estimated via linear regression on the annual means and then removed from the daily observations. If ``detrending_with_significance_test = True`` then a significance test is first used to determine whether the trend is to be removed. |brr|
    ...
    8. :py:func:`step8`: If ``scale_by_annual_cycle_of_upper_bounds = True`` (for rsds only) then a debiased annual cycle of upper bounds is calculated to rescale the debiased climate model values from [0,1] back to the original scale. |brr|
    ...
    However depending on the debiasing region and usecase at hand it might be important to modify some of the default settings. This can be done using something like
    ...
    If run with the default options results are exactly identical to v3.0.1 (2022-06-27) of the `ISIMIP3BASD reference implementation <https://zenodo.org/record/6758997/>`_. By setting parameters appropriately reproducing lower versions is possible and also consistency with future versions is planned.

    In contrast to the reference implementation this code includes further options for customization of the ISIMIP behavior, allows more flexible usage given that it operates on a numerical basis and handles a variety of cases that might lead to bugs in the reference implementation (TODO).
    ...
    **Full list of attributes:**
    ...
    upper_threshold : float
        Upper threshold of a variable. Default: ``np.inf``. |brr|
    ...
    window_length_annual_cycle_of_upper_bounds : int
        Window length to estimate an annual cycle of upper bounds to rescale. Only relevant if ``scale_by_annual_cycle_of_upper_bounds = True``. Default: ``31``. |brr|

    Step 2:
    impute_missing_values : bool
        Whether in step 2 missing values (indicated either by a mask of ``np.nan`` values) are imputed using the other values within the window. |brr|

    Step 3:
    detrending : bool
        Whether detrending is applied in step4 to account for within obs, cm_hist and cm_fut trends.
    detrending_with_significance_test : bool
        Whether detrending is applied conditional on a significance test for the trend. Only relevant if ``detrending = True``. Default: ``True``. |brr|

    Step 5:
    trend_preservation_method : str
        One of ``["additive", "multiplicative", "mixed", "bounded"]``. Method of how the climate change trend is transferred to observations to generate pseudo future ones.
    trend_transfer_only_for_values_within_threshold : bool
        Whether the trend transfer is done only for the values within thresholds. Default: ``True``. |brr|
    ...
    nonparametric_qm : bool
        Whether nonparametric quantile mapping is used for debiasing in step 6. If ``nonparametric_qm = False`` then quantile mapping is done using the distribution in ``distribution``.
    ...
    ks_test_for_goodness_of_cdf_fit : bool
        Whether a Kolmogorov Smirnov statistic is used to diagnose misfit of the parametric CDF prior to quantile mapping. If such misfit is diagnosed nonparametric quantile mapping is used. Only relevant if ``nonparametric_qm = False``. Default: ``True``. |brr|
    ...
    mode_non_parametric_qm : str
        One of ``["normal", "isimipv3.0"]``. Computation method used for nonparametric quantile mapping. If ``"normal"`` this is based on estimated empirical CDFs and inverse empirical CDFs. If ``"isimipv3.0"`` this is based on estimated ranks using ``scipy.stats.rankdata`` which can lead to inconsistencies. Default: ``"normal"``. |brr|
