...
standard_variables_isimip = {
    ...
    "tasrange": {
        ...
        "upper_bound": np.inf,
        "upper_threshold": np.inf,
        "distribution": scipy.stats.rice, # TODO: needs to be Rice
        "trend_preservation": "mixed",
        "detrending": False,
...
class ISIMIP(Debiaser):
    def __init__(self, variable):
        ...
        self.lower_bound = standard_variables_isimip[variable]["lower_bound"]
        self.lower_threshold = standard_variables_isimip[variable]["lower_threshold"]
        self.upper_bound = standard_variables_isimip[variable]["upper_bound"]
        self.upper_threshold = standard_variables_isimip[variable]["upper_threshold"]
        self.distribution = standard_variables_isimip[variable]["distribution"]
        self.trend_preservation = standard_variables_isimip[variable]["trend_preservation"]
    ...
    def step2(self, obs, cm_hist, cm_future):
        if self.variable == "prsnratio":
            ...
            obs_hist = np.where(np.isnan(obs_hist), iecdf_obs_hist(np.random.uniform(len(obs_hist))), obs_hist)
            cm_hist = np.where(np.isnan(cm_hist), iecdf_cm_hist(np.random.uniform(len(cm_hist))), cm_hist)
            cm_future = np.where(np.isnan(cm_future), iecdf_cm_future(np.random.uniform(len(cm_future))), cm_future)
    ...
    def step4(self, obs, cm_hist, cm_future):
        if self.lower_bound > -np.inf and self.lower_threshold > -np.inf:
            ...
            cm_future = np.where(
                ...
                self.lower_bound + (1 - np.random.powerlaw(a = 2))/(self.lower_threshold - self.lower_bound),
                cm_future
            )
        if self.upper_bound < np.inf and self.upper_threshold < n.inf:
            cm_future = np.where(
                cm_future >= self.upper_threshold,
                self.upper_threshold + np.random.powerlaw(a = 2)/(self.upper_bound - self.upper_threshold),
                cm_future
            )            
    ...
    def step5(self, obs_hist, cm_hist, cm_future):
        ...
        if self.trend_preservation == "additive":
            ...
            delta_mult = np.max(0.01, np.min(100, delta_star_mult))
            ...
            return_vals[condition1] = a + (obs_hist[condition1] - a)(q_cm_future[condition1] - a)/(q_cm_hist[condition1] - a)
            return_vals[condition2] = obs_hist[condition2]
            return return_vals
        else:
            raise ValueError("Wrong value for self.trend_preservation. Needs to be one of ['additive', 'multiplicative', 'mixed', 'bounded']")
       
    # TODO: Still need to do bound enforcing prior (and afterwards) (first part of step 6)
    def step6_map_values_between_thresholds(self, obs_hist_sorted, obs_future_sorted, cm_hist_sorted, cm_future_sorted):
        ...
        cdf_vals_cm_future = ISIMIP.apply_cdf_thresholding(self.distribution.cdf(cm_future_sorted, *fit_obs_hist))
        
        cdf_vals_obs_hist = ISIMIP.interp_cdf_values_on_len_cm_future(
            ISIMIP.apply_cdf_thresholding(self.distribution.cdf(obs_hist_sorted, *fit_obs_hist)), len(cdf_vals_cm_future)
        )
        cdf_vals_cm_hist = ISIMIP.interp_cdf_values_on_len_cm_future(
            ISIMIP.apply_cdf_thresholding(self.distribution.cdf(cm_hist_sorted, *fit_cm_hist)), len(cdf_vals_cm_future)
        )
        ...
        delta_log_odds = np.max(-np.log(10), np.min(np.log(10), L_cm_future - L_cm_hist))
        
        
        # Map values following formula 10
        mapped_vals = self.distribution.ppf(
            scipy.special.expit(L_obs_hist + delta_log_odds),
            *fit_obs_future
        )
        return mapped_vals
    ...
    def step6(self, obs_hist, obs_future, cm_hist, cm_future):
        ...
        if self.lower_threshold > -np.inf or self.upper_threshold < np.inf:
            ...
            if self.lower_threshold > - np.inf:
                P_obs_hist = (obs_hist_sorted < self.lower_threshold).sum() / len(obs_hist_sorted)
                P_cm_hist = (cm_hist_sorted < self.lower_threshold).sum() / len(cm_hist_sorted)
                P_cm_future = (cm_future_sorted < self.lower_threshold).sum() / len(cm_future_sorted)
                
                # TODO: round (3.5 --> 4) returns other value than np.rint (3.5 --> 3)
                nr_indices_to_set_to_lower_bound = round(len(cm_future_sorted)*P_obs_future) 
            
            nr_indices_to_set_to_higher_bound = len(cm_future_sorted)
            if self.upper_threshold < np.inf:
                P_obs_hist = (obs_hist_sorted > self.upper_threshold).sum() / len(obs_hist_sorted)
                P_cm_hist = (cm_hist_sorted > self.upper_threshold).sum() / len(cm_hist_sorted)
                P_cm_future = (cm_future_sorted > self.upper_threshold).sum() / len(cm_future_sorted)
    
                nr_indices_to_set_to_higher_bound = round(len(cm_future_sorted)*P_obs_future)
        
            cm_future_sorted[0:nr_indices_to_set_to_lower_bound] = self.lower_bound
            cm_future_sorted[len(cm_future_sorted)-nr_indices_to_set_to_higher_bound:] = self.higher_bound
            cm_future_sorted[nr_indices_to_set_to_lower_bound:len(cm_future_sorted)-nr_indices_to_set_to_higher_bound] = self.step6_map_values_between_thresholds(
                obs_hist_sorted[(obs_hist_sorted > self.lower_threshold) & (obs_hist_sorted < self.upper_threshold)], 
                obs_future_sorted[(obs_future_sorted > self.lower_threshold) & (obs_future_sorted < self.upper_threshold)], 
                cm_hist_sorted[(cm_hist_sorted > self.lower_threshold) & (cm_hist_sorted < self.upper_threshold)], 
                cm_future_sorted[nr_indices_to_set_to_lower_bound:len(cm_future_sorted)-nr_indices_to_set_to_higher_bound]
            )
            ...
            reverse_sorting_idx = np.argsort(cm_future_argsort)
            cm_future_sorted = self.step6_map_values_between_thresholds(obs_hist_sorted, obs_future_sorted, cm_hist_sorted, cm_future_sorted)
            return cm_future_sorted[reverse_sorting_idx]
