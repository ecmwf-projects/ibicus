...
def rmse_spatial_correlation_distribution(variable: str, obs_data: np.ndarray, **cm_data) -> pd.DataFrame:
    ...
    for k in cm_data.keys():
        ...
        rmsd_arrays.append(
            pd.DataFrame(data={"Correction Method": [k] * rmsd.size, "RMSE spatial correlation": rmsd.flatten()})
        )
...
def calculate_corr_bs_replicates(data: pd.DataFrame, size: int):
    """
    Bootstrapped estimates to obtain estimates of Pearson correlation coefficient in specified dataframe.
