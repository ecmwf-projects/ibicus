...
def _calculate_quantile_trend_bias(
    ...
    if trend_type == "additive":
        ...
        if np.quantile(bc_validate, quantile, axis=0) == 0 or np.quantile(raw_validate, quantile, axis=0):
            ...
            raise Warning(
                "Selected quantile is zero, cannot analyse multiplicative trend. Output will be automatically set to zero."
            )
...
def calculate_future_trend_bias(
    ...
    for k in debiased_cms.keys():
        ...
        mean_bias = _calculate_mean_trend_bias(variable, "additive", raw_validate, raw_future, *debiased_cms[k])
...
def plot_future_trend_bias(variable: str, bias_array: np.ndarray):
    ...
    if bias_array.shape[1] != 3:
        raise TypeError(
            "Wrong input data format. Needs three columns Bias correction method, Metric, Bias value at certain location]. Use function calculate_future_trend_bias to generate input."
        )
