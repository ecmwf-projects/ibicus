...
def _calculate_metrics_trend_bias(
    ...
    if trend_type == "additive":
        bc_trend = metric.calculate_exceedance_probability(bc_future) - metric.calculate_exceedance_probability(bc_validate)
        raw_trend = metric.calculate_exceedance_probability(raw_future) - metric.calculate_exceedance_probability(raw_validate)

    elif trend_type == "multiplicative":
        if (m_bc_validate := metric.calculate_exceedance_probability(bc_validate)) != 0 and (
            m_raw_validate := metric.calculate_exceedance_probability(bc_validate)) != 0:
...
def calculate_future_trend_bias(variable: str, raw_validate: np.ndarray, raw_future: np.ndarray, metrics: list = [], trend_type: str = 'additive', **debiased_cms) -> pd.DataFrame:
    ...
    for debiased_cms_key, debiased_cms_value in debiased_cms.items():
        ...
        for m in metrics:
            ...
            metric_bias = _calculate_metrics_trend_bias(variable, trend_type, m, raw_validate, raw_future, *debiased_cms_value)
...
def plot_future_trend_bias_spatial(variable: str, metric: str, bias_df: pd.DataFrame, manual_title: str = " "):
    ...
    if metric not in bias_df['Metric'].unique():
        raise ValueError('Chosen metric not calculated in dataframe given as input. Either change the metric argument, or re-calculate the dataframe and include the metric of your choice.')
