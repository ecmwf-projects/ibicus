...
def cdf_precipitation_hurdle_model(x, fit, distribution = scipy.stats.gamma, randomization = False):
    ...
    if not randomization: 
        ...
        return np.where(x == 0, np.random.uniform(0, p0), p0 + (1-p0)*distribution.cdf(x, *fit_rainy_days))
...
def quantile_mapping_precipitation_hurdle_model(x, fit_right, fit_left, distribution_right = scipy.stats.gamma, distribution_left = scipy.stats.gamma, randomization = False):
