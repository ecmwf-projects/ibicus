...
2. Ibicus is not suitable for 'downscaling' the climate model which is a term for methods used to increase the spatial resolution of climate models. Although bias corrections methods have been used for downscaling, in general they are not appropriate, since they do not reproduce the local scale variability that is crucial on those scales. Maraun paper argue that for downscaling, stochastic methods have great advantages. Examples of packages addressing the problem of downscaling are: `Rglimclim <https://www.ucl.ac.uk/~ucakarc/work/glimclim.html>`_ Link to downscaling packages. |brr|
